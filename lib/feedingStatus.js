...
export function getStarterFeedingStatus(starter, feedings = []) {
  ...
  if (hoursUntilFeed <= 0) {
    ...
    if (Math.abs(hoursUntilFeed) >= intervalHours) {
      ...
      isOverdue = true;
      statusText = "Dringend füttern!";
      statusEmoji = "🚨";
      statusColor = "danger";
    } else {
      isDue = true;
      statusText = "Fütterung fällig";
      statusEmoji = "⏰";
      statusColor = "warning";
    }
    ...
    isComingUp = true;
    statusText = `Bald füttern (in ${Math.round(hoursUntilFeed)}h)`;
    statusEmoji = "⏳";
    statusColor = "soon";
  } else {
    // Alles okay
    if (inFridge) {
      ...
      statusText = `Nächste Fütterung in ${Math.round(hoursUntilFeed)}h`;
    }
